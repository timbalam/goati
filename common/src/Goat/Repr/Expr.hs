...
componentsBlockFromDeclares
 :: MonadBlock (Abs Components) m
 => Declares a
 -> ( (Local [Text], Public (Components ()))
    , Bindings
       (Parts (Pattern p) Components)
       (Pattern Components)
...
componentsBlockFromDeclares (Declares (Local lr) (Public pr) k) =
  ...
  where
    ...
    (lp, lms) =
      fieldsReprsFromNode (reprFromAssigns . k <$> lr)
...
reprFromNode p bs i = [Scope (wrapBlock abs)] where
  abs =
    Abs
      (hoistBindings lift
        (letParts (Decomp p (B (Local i))) (F . return <$> bs)))
{-
...
abstractLocal ns =
  abstract (\case
    Left (Local n) -> Local <$> elemIndex n ns
    Right _ -> Nothing)
