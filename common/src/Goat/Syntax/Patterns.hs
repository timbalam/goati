...
instance (Traversable f, S.Patt (f (Maybe s)), S.Esc a)
...
instance (S.Self k, Ord k, S.RelPath a, S.LocalPath a)
  => S.Block (Patt (Decomps k) (Maybe a)) where
  type Stmt (Patt (Decomps k) (Maybe a)) =
    Matching k (Patt (Decomps k) (Maybe a))
    
  block_ ts = Nothing :< S.block_ ts
  
instance (S.Self k, Ord k, S.RelPath a, S.LocalPath a)
  => S.Block (Decomp (Decomps k) a) where
  type Stmt (Decomp (Decomps k) a) = Matching k a
